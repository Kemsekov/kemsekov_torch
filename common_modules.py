...
class UpscaleResize(nn.Module):
    ...
    """
    def __init__(self, in_ch, out_ch, scale_factor, dimensions=2, mode='nearest'):
        ...
        if in_ch == out_ch:
            ...
            if dimensions == 1:
                self.channel_adjust = nn.Conv1d(in_channels=in_ch, out_channels=out_ch, kernel_size=1)
            elif dimensions == 2:
                self.channel_adjust = nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=1)
            elif dimensions == 3:
                self.channel_adjust = nn.Conv3d(in_channels=in_ch, out_channels=out_ch, kernel_size=1)
