...
class CRF(nn.Module):
    """
    Compilable by torchscript version of CRF module.
    ...
    def __init__(self, n_spatial_dims, filter_size=11, n_iter=5, requires_grad=True,
        ...
        self.n_iter = n_iter
        self.filter_size = np.broadcast_to(filter_size, n_spatial_dims).tolist()
    ...
    def forward(self, x, spatial_spacings: Optional[torch.Tensor]=None):
        """
        Parameters
        ----------
        x : torch.tensor
        ...
        """
        batch_size, n_classes = x.shape[:2]
        spatial = x.shape[2:]
    ...
    @staticmethod
    def _gaussian_filter(x, inv_theta, spatial_spacing, filter_size : List[int]):
        """
        Parameters
        ----------
        x : torch.tensor
        ...
        for i, dim in enumerate(range(1, x.ndim)):
            ...
            new_shape = list(shape_before_flatten)+[x.shape[-1]]
            x = x.squeeze(1).view(new_shape).transpose(-1, dim)

        return x

    @staticmethod
    def create_gaussian_kernel1d(inverse_theta, spacing, filter_size : int):
        """
        Parameters
        ----------
